...
function [PE_tot,force] = NNG98_2_coordStore_PBC_NEW_Iter01(count,inputVectorType,coord,total,numMov,movAtom)
...
%%####  ------------------------    Following is TEMPORARY modification To store configs with C-H > 3 and C-Br > 4  ---------------------------------
%             if((in(1) > 3.0) | (in(3) > 3.0) | (in(4) > 3.0) | (in(6) > 3.0) | (in(9) > 4.0)) %%### TEMPORARY To store configs with C-H > 3 and C-Br > 4
% 
...
%             end
%%####  ------------------------    Above is TEMPORARY modification To store configs with C-H > 3 and C-Br > 4  -----------------
            

            if(in(6) > 2.5 & in(12) < 2.0 & in(9) > 3.0 )
...
%                 flag_largeBondDist_G98Discont = 1;
            ...
            if((sum(in < minin) > 0) | sum((in > maxin) > 0))
                ...
                if((in(1) < 3.0) & (in(3) < 5.0) & (in(4) < 5.0) & (in(6) < 5.0) & (in(9) < 4.0))
                    %                     effectiveDist = sqrt(sum((in-minin).^2.*(maxin-in).^2));
                    ...
                    %------Following is the stopping sondition for the antire simulation
%                 else if((in(1) > 3.0) | (in(3) > 3.0) | (in(4) > 3.0) | (in(6) > 3.0) | (in(9) > 4.0))
                else if((in(1) > 3.0) | (in(3) > 5.0) | (in(4) > 5.0) | (in(6) > 5.0) | (in(9) > 4.0) | (in(6) > 2.5 & in(12) < 2.0 & in(9) > 3.0 ))
